...
const ENGLISH = new Map(Object.entries({
  ...
  "刷新失败": "Refresh failed",
  "正在刷新…": "Refreshing…",
  "刷新失败，显示上次状态": "Refresh failed; showing the last successful state",
  "刷新失败，显示 {time} 状态": "Refresh failed; showing the state from {time}",
  "状态不是最新数据，暂不可排序": "State is stale; sorting is temporarily unavailable",
  "当前状态不是最新数据，请先刷新": "The current state is stale. Refresh before trying again.",
  "立即刷新": "Refresh now",
  ...
  "填写后，只有收到有效 HTTP 响应并经过稳定确认后才会显示“已连接”。每次失败后间隔 3 秒重试，连续失败最多重试 10 次；稳定连接后计数自动清零。留空时仅检查本地 TCP 监听。": "When configured, the tunnel is marked Connected only after a valid HTTP response and a stability window. Retries wait 3 seconds, with up to 10 consecutive failures; the counter resets after a stable connection. Leave blank to check only the local TCP listener.",
  "当前连续失败次数": "Current consecutive failures",
  "自动重试上限": "Automatic retry limit",
  ...
  "入口未就绪": "Entry Not Ready",
  "远端服务尚未就绪": "Remote service is not ready",
  "连接中": "Connecting",
