...
- 后台轮询的 `refreshing` 只是正在取新快照，不应禁用仍基于最后成功快照可安全执行的托盘操作；真正 `stale/offline` 时才禁用写操作。

## macOS PF 无端口访问

- `pfctl -e` 只打开 PF，不会自动恢复 `/etc/pf.conf` 的主规则。Local Ops 子 anchor 丢失时必须先恢复 Apple 主规则，再通过显式 request 让 root helper 加载 Local Ops anchor。
- 只有 Caddy 内部端口健康、回环端口 80 不可用、无端口访问已启用且 helper/daemon/anchor 与当前配置同步时，才能判定为 PF 故障。业务 5xx、SSH 隧道失败或端口 80 被其他 HTTP 服务占用都不得触发 PF 修复。
- PF 修复必须有单飞、冷却、时间窗预算和持久状态；App 重启不能重置尝试额度，也不能把失败的修复提示为成功。
