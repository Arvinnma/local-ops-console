...
const cssOutputPath = path.join(root, "public", "icon-library.css");
...
const cssOutput = `/* Generated by scripts/generate-icon-library.mjs. Do not edit by hand. */\n${icons
  .map((item) => `.icon-color-${cssToken(item.id)} { --resource-icon-color: #${item.hex}; --resource-icon-soft: #${item.hex}16; color: #${item.hex}; }`)
  .join("\n")}\n`;
fs.writeFileSync(cssOutputPath, cssOutput);
console.log(`已生成 ${icons.length} 个本地图标：${outputPath}、${cssOutputPath}`);
